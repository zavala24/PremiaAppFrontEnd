...
export default function GlobalHamburger({ show = true }: Props) {
  ...
  return (
    <>
      ...
      <Animated.View pointerEvents={open ? "auto" : "none"} style={overlayStyle}>
        <Pressable style={StyleSheet.absoluteFill} onPress={() => setOpen(false)} />
      </Animated.View>

      <Animated.View style={drawerStyle} pointerEvents={open ? "auto" : "none"}>
        <LinearGradient
        ...
        <View style={[styles.bottom, { paddingBottom: insets.bottom + 12 }]}>
          <Pressable onPress={goLogout} style={({ pressed }) => [styles.logout, pressed && { opacity: 0.7 }]}>
            <MaterialCommunityIcons name="logout" size={20} color="#FFFFFF" />
            <Text style={[styles.logoutText, { color: "#FFFFFF" }]}>Cerrar sesión</Text>
          </Pressable>
        </View>
      </Animated.View>
    </>
...
function MenuItem({
  ...
  return (
    <Pressable onPress={onPress} style={styles.item}>
      ...
      <MaterialCommunityIcons name="chevron-right" size={22} color="#FFFFFF99" style={{ marginLeft: "auto" }} />
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  bubble: { position: "absolute", height: 160, width: 160, borderRadius: 80, backgroundColor: "#FFFFFF" },
  headerRow: { flexDirection: "row", alignItems: "center", paddingVertical: 12 },
  avatar: { height: 34, width: 34, borderRadius: 17, backgroundColor: "#ffffff22", justifyContent: "center", alignItems: "center" },
  closeBtn: { height: 34, width: 34, borderRadius: 17, backgroundColor: "#fff", alignItems: "center", justifyContent: "center", marginLeft: 8 },
  title: { color: "#fff", fontWeight: "800", fontSize: 16 },
  subtitle: { color: "#ffffffbb", fontSize: 12, marginTop: 2 },
  section: { paddingHorizontal: 16, paddingTop: 8 },
  sectionLabel: { color: "#ffffff99", fontSize: 12, letterSpacing: 1, marginBottom: 8 },
